...
CMd5Encode::CMd5Encode() {}


CMd5Encode::~CMd5Encode() {}

void CMd5Encode::mainLoop(unsigned int M[]) {
	unsigned int f, g;
...
*最后加上64位的原来长度
*/
unsigned int* CMd5Encode::add(string& str) {
...
string CMd5Encode::changeHex(int a) {
	int b;
	string str1;
	string str = "";
	for (int i = 0; i < 4; i++) {
...
string CMd5Encode::getMD5(string source) {
	atemp = A;    //初始化
	btemp = B;
	ctemp = C;
	dtemp = D;
